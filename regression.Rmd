...
bind_rows(mutate(all_cv_preds, type = "All"),
  ...
  geom_point() + coord_flip() + theme_bw() + xlab("Model Type") + ylab("Root Mean Squared Error")
```

All the three models have a better prediction accuracy than the null model. The next plot is a similar plot to this but without the null model in order to better compare these models
...
The model with all selected variables and no interactions had the lowest rmse. 
